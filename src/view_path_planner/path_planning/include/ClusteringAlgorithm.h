...
class MSCluster : public MeanShift {
  public:
    ...
    void getMeanShiftClusters(vector<geometry_msgs::Point> &points,
      vector<geometry_msgs::Point> &cluster_points, std::vector<double> &contain_points, double kernelBandwidth)
    {
      ...
      for (int i = 0 ; i < clusters.size(); i++)
      {
        ...
        for (int j = 0; j < points.size(); j++)
        {
          contain_points.push_back(calculateDistance(tempPoint, points[j]));
        }
        // Find min element
        int minElementIndex = std::min_element(
        contain_points.begin(), contain_points.end()) - contain_points.begin();
        cluster_points.push_back(points[minElementIndex]);
      }
    }
