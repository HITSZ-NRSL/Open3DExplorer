...
class ColorPclGenerator:
    ...
    \date May - July 2018
    '''
    def __init__(self, intrinsic, width = 640, height = 480, frame_id = "/camera_color_optical_frame", point_type = PointType.SEMANTICS_BAYESIAN):
        '''
    ...
    def generate_cloud_data_common(self, bgr_img, depth_img):
        ...
        """
        bgr_img = bgr_img.view('<u1')       #numpy的函数
        depth_img = depth_img.view('<f4')
        
        depth_img = np.where(depth_img == 0, 11000.0, depth_img)    # ! 仿真里无穷远处的光线投射至11m，可以解决octoserver原点处存在杂点的问题
        
        # Add depth information
        self.xyd_vect[:,0:2] = self.xy_index * depth_img.reshape(-1,1)/1000      # 转换成1行
        # print(len(self.xyd_vect[:,0:2]))
        # print(self.xyd_vect[:,0:2])
        self.xyd_vect[:,2:3] = depth_img.reshape(-1,1)/1000   # 转换成1列，行数根据数据量来
        # print(len(self.xyd_vect[:,2:3]))
        
        # self.xyd_vect[:,0:3] = np.where(self.xyd_vect[:,2:3] >= 8, [0.0,0.0,0.0], self.xyd_vect[:,0:3])    # ! 点云截断for FNBVE
        
        # print(len(self.xyd_vect[:,2:3]))
        # print(self.xyd_vect[:,2:3])
    ...
    def generate_cloud_semantic_max(self, bgr_img, depth_img, semantic_color, confidence, stamp):
        ...
        # Concatenate data
        self.ros_data[:,4:5] = self.semantic_color_vect.view('<f4')   # semantic_color to color
